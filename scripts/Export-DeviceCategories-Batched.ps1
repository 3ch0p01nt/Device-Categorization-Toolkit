...
| extend SecondaryCategory = iff(array_length(AllCategories) > 1, strcat_array(array_slice(AllCategories, 1, array_length(AllCategories)), ", "), ""), RoleCount = array_length(AllCategories)
| project DeviceName, DeviceId, Tier, Category, SecondaryCategory, RoleCount, Confidence, DetectionMethod, Evidence, NativeDeviceType, OSPlatform, OSVersion, MachineGroup, LastSeen, IsAzureADJoined, JoinType
"@
